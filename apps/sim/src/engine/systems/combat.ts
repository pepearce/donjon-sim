...
import { itemsOf, livingRoster, monstersIn, roster } from '../world.js';
import { BLEED_OUT_TICKS, clamp, statMod, type Hero, type Monster, type Team, type World } from '../types.js';
import { awardXp } from './progression.js';
import { dropLoot } from './loot.js';

export function heroAtk(world: World, hero: Hero): number {
  const gear = itemsOf(world, hero).reduce((n, i) => n + i.atk, 0);
  return 6 + Math.floor(hero.level * 0.6) + statMod(hero.stats[hero.primary]) + gear;
}

export function heroDef(world: World, hero: Hero): number {
  const gear = itemsOf(world, hero).reduce((n, i) => n + i.def, 0);
  return 6 + Math.floor(hero.level * 0.4) + statMod(hero.stats.agi) + gear;
}
...
function downHero(world: World, hero: Hero, team: Team, source: string): void {
  ...
  team.morale = clamp(0, 100, team.morale - 12);
  emit(world, {
    type: 'HERO_DOWN',
    teamId: team.id,
    heroId: hero.id,
...
export function resolveCombatRound(world: World, team: Team): void {
  ...
  for (const actor of order) {
    if (actor.kind === 'hero') {
      ...
      const targetRng = rngFor(world.seed, world.tick, RngDomain.COMBAT_TARGET, hero.id);
      const target = targetRng.pick(targets);

      const hitRng = rngFor(world.seed, world.tick, RngDomain.COMBAT_HIT, hero.id);
      const d20 = hitRng.int(1, 20);
      if (d20 === 1) continue;
      const crit = d20 === 20;
      if (!crit && d20 < hitDc(heroAtk(world, hero), target.def)) continue;
...
export function attemptStabilise(world: World, team: Team): void {
  ...
  for (const hero of downed) {
    ...
    const p = clamp(
      0.1,
      0.9,
      0.45 + 0.05 * statMod(helper.stats.wil) + (helper.className === 'pretre' ? 0.15 : 0) + 0.02 * helper.level,
    );
    if (rng.chance(p)) {
      hero.state = 'ok';
      hero.hp = 1;
